...
export default function CommandPalette({ open, onClose }){
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-start justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-xl border" onClick={e=>e.stopPropagation()}>
        <div className="p-3">
          <input ref={inputRef} className="w-full p-2 rounded border text-sm" placeholder="Search by client or brief…" value={query} onChange={e=>setQuery(e.target.value)} />
        </div>
        <div className="max-h-80 overflow-auto divide-y">
          {results.map(c => (
            <div key={c.id} className="p-3 cursor-pointer hover:bg-slate-50" onClick={()=>handleSelect(c.id)}>
